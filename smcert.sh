...
usage() {
    echo "Usage: $0 [-lc|--list-certificates [<directory>]] [-mi|--mkcert-install] [-mu|--mkcert-uninstall] [-gca|--generate-ca] [-ica|--install-ca] [-gcrt|--generate-certificate] [-cuc|--check-used-certificate [<host_port>]] [-cvh|--create-virtual-host] [-v|--view [<directory>]] [--clear]"
    exit 1
}
...
create_apache_virtual_host() {
    ...
    local server_root=$(apachectl -S | awk '/ServerRoot/ {print $NF}' | sed 's/"//g')
    local apache_dir=${apache_dir:-"$server_root/sites-available"}
    ...
    local tmp_filename="/tmp/$filename"
    sudo cat <<EOF > "$tmp_filename"
<VirtualHost *:443>
    ServerName $local_ip
...
EOF
    ...
    sudo cp "$tmp_filename" "$apache_dir"
    rm -r "$tmp_filename"
...
while [ "$#" -gt 0 ]; do
    case "$1" in
        ...
        -cuc|--check-used-certificate)
            ...
            shift
            if [ "$#" -gt 0 ] && [[ ! "$1" =~ ^- ]]; then
                host_port_for_certificate="$1"
                shift
            else
                echo "Error: Please provide the host_port to view the certificate used."
                exit 1
            fi
            ;;
...
if [ "$check_used_certificate_flag" -eq 1 ]; then
    check_used_certificate "$host_port_for_certificate"
    exit 0
fi
...
if [ "$create_apache_virtual_host_flag" -eq 1 ]; then
    ...
    create_apache_virtual_host "$filename" "$backend_path" "$crt_path" "$key_path"
    exit 0
fi
